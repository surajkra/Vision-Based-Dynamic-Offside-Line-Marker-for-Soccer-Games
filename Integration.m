...
clc
%% Reading the Video
obj = VideoReader('My Great Game - My Great Capture - 2018-04-04 17-57-51.mp4');
v = VideoWriter('v_1','MPEG-4');
open(v);
for frame_index = 1 : 5000 %fill in the appropriate number
    disp(frame_index)
    img = readFrame(obj);    
    ...
    if(frame_index~=351 && mod(frame_index,20)~=0)
        continue;
    end
    BW_img = rgb2gray(img);
